...
void MouseEventListener::Update(SDL_Event e_){
	// Check what event was passed through.
...
glm::vec2 MouseEventListener::GetPreviousMousePosition(){ return prevMouse; }

glm::vec2 MouseEventListener::GetMousePosition(){ return mouse; }

// Invert Y (mouse should be starting from bottom not up.)
glm::vec2 MouseEventListener::GetMouseOffset(){ return glm::vec2(mouse.x - prevMouse.x, prevMouse.y - mouse.y); }
...
void MouseEventListener::UpdateMousePosition(){
	...
	if (firstUpdate) {
		prevMouse.x = mouse.x = static_cast<float>(tmpX);
		prevMouse.y = mouse.y = static_cast<float>(tmpY);
		firstUpdate = false;
	}
	else {
		prevMouse.x = mouse.x;
